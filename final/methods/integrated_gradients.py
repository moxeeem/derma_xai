...
# -*- coding: utf-8 -*-
"""
Integrated Gradients для CLIP-текста (без matplotlib), вертикальная узкая карточка.

Метод:
  IG_i = (embed_i - baseline_i) * ∫_{α∈[0,1]} ∂ cos(image, text(α)) / ∂ embed_i  dα
  Реализация: mid-point rule по шагам α=(k+0.5)/steps, baseline = 0.

Поддержка:
  • HF CLIP: через inputs_embeds (или hook на token_embedding, если старый transformers).
  • open_clip: патчим model.token_embedding.forward → out*α, накапливаем градиенты.
    Токены в слова маппим по словам подписи (без вывода id_*).
...
class IGConfig:
    ...
    # IG
    steps: int = 32                  # 16..64 — ок
    tau: float = 0.60                # для coverage по словам
...
def _coerce_cfg(config: Union[IGConfig, Dict[str, Any], None]) -> IGConfig:
    if isinstance(config, IGConfig):
        return config
    d = dict(config or {})
    valid = {f.name for f in fields(IGConfig)}
...
# --- open_clip: размапим токен-оценки на слова подписи (без id_…) ---
...
# ================== Text side features (HF / open_clip) =================

def _hf_tokens_and_ids(clip, text: str):
    tok = clip.tokenizer([text], return_tensors='pt', padding=True, truncation=True, max_length=77)
    return tok['input_ids'].to(clip.device), tok['attention_mask'].to(clip.device)

def _hf_text_embed_from_embeds(clip, emb: torch.Tensor, attn_mask: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
    tm = clip.model.text_model
    out = tm(inputs_embeds=emb, attention_mask=attn_mask, output_hidden_states=True, return_dict=True)
    ...
    pooled = tm.final_layer_norm(last_hidden.gather(1, idx).squeeze(1))
    tfeat = pooled @ clip.model.text_projection.weight.T
    return F.normalize(tfeat, dim=-1)

def _openclip_tokenize(clip, text: str):
    import open_clip
    tok_fn = open_clip.get_tokenizer(getattr(clip, "repo", "ViT-B-32"))
    toks = tok_fn([text]).to(clip.device)  # [1,77]
    return toks.squeeze(0)  # [T]


# ============================ Core IG ==================================

def compute_integrated_gradients(clip, image: Image.Image, text: str, cfg: IGConfig
                                 ) -> Tuple[List[str], List[float], Dict[str, Any]]:
    """
    Возвращает: words (для рендера), scores_word ∈ [0,1], meta (cos, steps, coverage и т.д.).
    """
    device = clip.device
    ...
    with torch.no_grad():
        if hasattr(clip, "encode_image"):
            ...
            px = clip.processor(images=[image], return_tensors='pt')['pixel_values'].to(device)
            img_feat = clip.model.get_image_features(pixel_values=px)
        img_feat = F.normalize(img_feat, dim=-1)

    steps = max(2, int(cfg.steps))
    alphas = torch.linspace(0.0, 1.0, steps, device=device) + (0.5/steps)  # mid-points
    alphas = torch.clamp(alphas, 0.0, 1.0)

    is_hf = hasattr(clip, "model") and hasattr(clip.model, "text_model")
    used_backend = "hf" if is_hf else "open_clip"

    if is_hf:
        # ---- HF path ----
        input_ids, attn_mask = _hf_tokens_and_ids(clip, text)
        tm = clip.model.text_model

        # исходные эмбеддинги (inputs_embeds предпочтительнее)
        try:
            tok_emb0 = tm.embeddings.token_embedding(input_ids)  # [1,T,dim]
            supports_inputs = True
        except Exception:
            supports_inputs = False

        if supports_inputs:
            tok_emb0 = tok_emb0.detach()
            total = torch.zeros_like(tok_emb0)
            for a in alphas:
                emb = (a * tok_emb0).detach().requires_grad_(True)  # baseline=0
                tfeat = _hf_text_embed_from_embeds(clip, emb, attn_mask, input_ids)
                clip.model.zero_grad(set_to_none=True)
                sim = cosine_sim(img_feat, tfeat)   # [1]
                sim.backward()
                g = emb.grad if emb.grad is not None else torch.zeros_like(emb)
                total = total + g
            ig_tok = (tok_emb0 * (total / steps)).sum(dim=-1).squeeze(0)  # [T]
        else:
            # старый transformers: патчим forward токен-эмбеддинга
            emb_mod = tm.embeddings.token_embedding
            orig_forward = emb_mod.forward

            with torch.no_grad():
                store0 = {}
                def hook0(_m, _inp, out): store0['emb'] = out
                h0 = emb_mod.register_forward_hook(hook0)
                _ = tm(input_ids=input_ids, attention_mask=attn_mask)
                h0.remove()
                tok_emb0 = store0['emb'].detach()  # [1,T,dim]

            acc = None
            for a in alphas:
                def patched_forward(ids):
                    out = orig_forward(ids) * a
                    patched_forward.last_out = out
                    out.retain_grad()
                    return out
                emb_mod.forward = patched_forward

                out = tm(input_ids=input_ids, attention_mask=attn_mask, output_hidden_states=True, return_dict=True)
                last_hidden = out.last_hidden_state
                eos_id = getattr(clip.tokenizer, "eos_token_id", None)
                if eos_id is None: eos_pos = (attn_mask.sum(dim=1) - 1).clamp(min=0).long()
                else: eos_pos = (input_ids == eos_id).int().argmax(dim=1)
                idx = eos_pos.view(-1,1,1).expand(-1,1,last_hidden.size(-1))
                pooled = tm.final_layer_norm(last_hidden.gather(1, idx).squeeze(1))
                tfeat = F.normalize(pooled @ clip.model.text_projection.weight.T, dim=-1)

                clip.model.zero_grad(set_to_none=True)
                sim = cosine_sim(img_feat, tfeat)
                sim.backward()
                g = patched_forward.last_out.grad
                acc = g.clone() if acc is None else (acc + g)

            emb_mod.forward = orig_forward
            ig_tok = (tok_emb0 * (acc / steps)).sum(dim=-1).squeeze(0)  # [T]

        # mask паддингов и чистим спец-токены
        pad_mask = _hf_tokens_and_ids(clip, text)[1].squeeze(0).bool()
        ig_tok = ig_tok[pad_mask]
        ids = input_ids.squeeze(0)[pad_mask]
        ids_list = ids.tolist()
        raw_tokens = clip.tokenizer.convert_ids_to_tokens(ids_list) if ids.numel() > 0 else []
        eos_id = getattr(clip.tokenizer, "eos_token_id", None)
        if cfg.drop_special_tokens and ids.numel() > 0 and eos_id is not None:
            keep_sc, keep_tok = [], []
            for s, i, t in zip(ig_tok.detach().cpu().tolist(), ids_list, raw_tokens):
                if i == eos_id: continue
                keep_sc.append(s); keep_tok.append(t)
            ig_tok = torch.tensor(keep_sc, device=ig_tok.device) if keep_sc else torch.zeros(0, device=ig_tok.device)
            raw_tokens = keep_tok

        arr_tok = ig_tok.detach().cpu().numpy().astype(np.float32)
        tok_scores01 = normalize01(np.abs(arr_tok))
        ...
        try:
            ...
            raise RuntimeError("open_clip is required for Integrated Gradients (text) on this model.")

        ids = _openclip_tokenize(clip, text)                  # [T]
        token_embed = clip.model.token_embedding
        orig_forward = token_embed.forward

        with torch.no_grad():
            base_emb = orig_forward(ids.unsqueeze(0))         # [1,T,dim]
        acc = None
        for a in alphas:
            def patched_forward(inp_ids):
                out = orig_forward(inp_ids) * a
                patched_forward.last_out = out
                out.retain_grad()
                return out
            token_embed.forward = patched_forward

            tfeat = clip.model.encode_text(ids.unsqueeze(0))  # [1,D]
            ...
            g = patched_forward.last_out.grad  # [1,T,dim]
            acc = g.clone() if acc is None else (acc + g)

        token_embed.forward = orig_forward

        ig_tok = (base_emb * (acc / steps)).sum(dim=-1).squeeze(0)  # [T]
        pad_mask = (ids > 0)
        ig_tok = ig_tok[pad_mask]
        arr_tok = ig_tok.detach().cpu().numpy().astype(np.float32)
        tok_scores01 = normalize01(np.abs(arr_tok))

        # маппим на слова подписи (без id_…)
        words, scores_word = _map_token_scores_to_words_by_length(text, tok_scores01)

    scores_word = normalize01(scores_word)

    # общий cos для метрик
    with torch.no_grad():
        if hasattr(clip, "encode_text"):
            tfeat_full = clip.encode_text([text])
        else:
            # HF вариант
            px = clip.tokenizer([text], return_tensors='pt', padding=True, truncation=True, max_length=77).to(device)
            feats = clip.model.get_text_features(**px)
            tfeat_full = F.normalize(feats, dim=-1)
        cos_full = float(cosine_sim(img_feat, tfeat_full)[0].item())
    ...
    meta = {
        ...
        "tokens_count": int(tok_scores01.size if 'tok_scores01' in locals() else 0),
...
def _measure_total_height(W: int, cfg: IGConfig, gen_caption: str, ref_caption: Optional[str],
    ...
    total += _text_size(drw, "Generated caption (Integrated Gradients)", f_hdr)[1] + 6
    # оценка высоты слов
...
def _render_card_vertical(out_png: Path, image: Image.Image,
                          ...
                          meta: Dict[str, Any], cfg: IGConfig):
    ...
    title = "Generated caption (Integrated Gradients)"
    ...
    kv("Бэкенд/шаги", f"{meta.get('backend','?')} / {int(meta.get('steps',0))}")
    y += GAP
    ...
    # футер
    footer = "Integrated Gradients • CLIP text attribution • cos(image, text)"
...
def run(clip, captioner, *, image_path: str, out_dir: Path,
        ...
        config: Optional[Union[IGConfig, Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    words, scores, meta = compute_integrated_gradients(clip, image, gen_caption, cfg)
